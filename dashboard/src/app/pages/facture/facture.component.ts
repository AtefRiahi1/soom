...
export class FactureComponent {
  ...
  addForm: FormGroup;
        ...
        private fetchEmployeProfile(email: string): void {
          ...
          this.getAllFactures(data.entreprise.id);
        ...
        getAllFactures(idEnt: number): void {
        ...
        onAddFacture(): void {
          if (this.addForm.valid) {
            ...
            this.factureService.createFacture(formData,this.user.entreprise.id, this.addForm.value.clientId,this.user.email) // Replace with actual data
              .subscribe({
                next: () => {
                  this.save();
                  Swal.fire('Ajouté!', 'La facture a été ajouté avec succès.', 'success');
                  this.getAllFactures(this.user.entreprise.id);
                  this.add?.hide();
                  this.addForm.reset();
                },
                error: error => {
        ...
        editModal(facture: any): void {
                ...
                this.editForm.patchValue({
                  ...
                  tva: facture.tva,
                  clientId: facture.client ? facture.client.id : null
                });
            ...
            if (hasInvalidQuantity) {
              ...
              return;
                  ...
                  this.factureService.updateFacture(this.selectedFacture.id, formData, this.user.email).subscribe({
                    next: () => {
                      this.saveEdit();
              ...
              searchEmployeBy(): void {
                if (this.searchArticle.trim() === '') {
                  ...
                  this.factures = this.factures.filter(n =>
                    n.numFacture.toLowerCase().includes(this.searchArticle.toLowerCase()) || n.client.nom.toLowerCase().includes(this.searchArticle.toLowerCase())
                  );
                }
              }
            ...
            getCRMPermissions(user:any) {
              ...
              convertToSortie(facture: any): void {
                ...
                this.sortieService.convertirFactureEnSortie(facture.id,this.user.email).subscribe({
                    next: (response) => {
                        this.savesortie(response);
                        Swal.fire('Succès!', 'Facture convertie en sortie.', 'success');
                        this.getAllFactures(this.user.entreprise.id); // Refresh the list if needed
                    },
                    error: (error) => {
              ...
              savesortie(reception:any): void {
                // Récupérer les valeurs du formulaire
                const factureNumber = reception.numSortie; // Use numCommande from the form
                ...
                this.clientService.getClientById(supplierId).subscribe(supplier => {
                  // Create the PDF
                  const pdfContent = this.generatePdfContentsortie(factureNumber, supplier, vat, products);
                  const pdfDoc = pdfMake.createPdf(pdfContent);
              ...
              generatePdfContentsortie(orderNumber: string, supplier: any, vat: number, products: any[]) {
                const prixHorsTaxe = products.reduce((acc, product) => acc + product.quantite * product.prixUnitaire, 0);
                const taxe = (prixHorsTaxe * vat) / 100;
                const prixPayer = prixHorsTaxe + taxe;
                ...
                return {
                  content: [
                    ...
                    {
                      text: 'Informations de la Sortie',
                      style: 'sectionHeader',
                      margin: [0, 10, 0, 10]
                    },
                    {
                      columns: [
                        [
                          { text: `Numéro de la sortie : ${orderNumber}`, bold: true },
                          { text: `Client : ${supplier.nom}`, bold: true },
                          { text: `Email : ${supplier.email}` },
                          { text: `Téléphone : ${supplier.tel}` },
                          { text: `Adresse : ${supplier.adresse}` }
