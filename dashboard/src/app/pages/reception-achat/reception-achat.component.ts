...
@Component({
  selector: 'app-reception-achat',
  templateUrl: './reception-achat.component.html',
  styleUrls: ['./reception-achat.component.scss']
})
export class ReceptionAchatComponent {
  ...
  receptions: any[] = [];
      ...
      selectedReception: any; // pour stocker le module sélectionné pour l'édition
      ...
      constructor(
        ...
        this.addForm = this.fb.group({
          numReception: ['', Validators.required],
      ...
      private fetchEmployeProfile(email: string): void {
        this.employeService.getEmployeByEmail(email).subscribe(
          (data) => {
            this.user = data;
            this.getAllReception(data.entreprise.id);
      ...
      getAllReception(idEnt: number): void {
        this.receptionService.getReceptionsByEntrepriseId(idEnt).subscribe(receptions => {
      ...
      onAddReception(): void {
        if (this.addForm.valid) {
          ...
          formData.nomFichier = `${formData.numReception}.pdf`;
      
          this.receptionService.createReceptionAchat(formData, this.user.entreprise.id, this.addForm.value.idFournisseur, this.user.email).subscribe({
            next: () => {
              this.save();
              Swal.fire('Ajouté!', "La réception a été ajoutée.", 'success');
              this.getAllReception(this.user.entreprise.id);
              this.add?.hide();
            },
            error: error => {
              console.error(error);
      ...
      editModal(reception: any): void {
        this.selectedReception = reception;
      
        this.editForm.patchValue({
          numReception: reception.numReception,
          tva: reception.tva,
          idFournisseur: reception.fournisseur ? reception.fournisseur.id : null
        });
        ...
        reception.produits.forEach((prod: any) => {
      ...
      onUpdateReception(): void {
        if (this.editForm.valid) {
          const formData = { ...this.editForm.value };
          formData.nomFichier = `${formData.numReception}.pdf`;
  
    
          this.receptionService.updateReceptionAchat(this.selectedReception.id, formData, this.user.email).subscribe({
            next: () => {
              this.saveEdit();
              Swal.fire('Modifié!', "La réception a été modifiée.", 'success');
              this.getAllReception(this.user.entreprise.id);
              this.edit?.hide();
            },
            error: error => {
      ...
      deleteModule(factureId: number): void {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            this.receptionService.deleteReceptionAchat(factureId).subscribe({
              next: () => {
                Swal.fire('Supprimé!', "La réception a été supprimée.", 'success');
                this.getAllReception(this.user.entreprise.id);
              },
              error: error => {
                console.error(error);
      ...
      searchEmployeBy(): void {
        if (this.searchArticle.trim() === '') {
          ...
          this.receptions = this.receptions.filter(n =>
            n.numReception.toLowerCase().includes(this.searchArticle.toLowerCase()) || n.fournisseur.nom.toLowerCase().includes(this.searchArticle.toLowerCase())
          );
        }
      }
      ...
      onDownloadFile(filename:string,numFacture:string){
        this.receptionService.downloadFile(filename,this.user.email).subscribe(
          event=>{
            console.log(event);
      ...
      save(): void {
        ...
        const orderNumber = this.addForm.value.numReception; // Use numCommande from the form
      ...
      saveEdit(): void {
        // Récupérer les valeurs du formulaire
        const orderNumber = this.editForm.value.numReception; // Use numCommande from the form
