...
export class DevisComponent {
  ...
  private fetchEmployeProfile(email: string): void {
    ...
    this.getAllDevis(data.entreprise.id);
  ...
  getAllDevis(idEnt: number): void {
    this.devisService.getDevisByEntrepriseId(idEnt).subscribe(devis => {
      this.devis = devis;
    });
  }
  ...
  onAddDevis(): void {
    if (this.addForm.valid) {
      ...
      const formData = { ...this.addForm.value };
          formData.nomFichier = `${formData.numDevis}.pdf`;

      this.devisService.createDevis(formData,this.user.entreprise.id, this.addForm.value.clientId,this.user.email) // Replace with actual data
        .subscribe({
          next: () => {
            this.save();
            Swal.fire('Ajouté!', 'Le devis a été ajouté avec succès.', 'success');
            this.getAllDevis(this.user.entreprise.id);
            this.add?.hide();
            this.addForm.reset();
          },
          error: error => {
            console.error(error);
            Swal.fire('Erreur!', 'Une erreur est survenue lors de l\'ajout du devis.', 'error');
          }
        });
    }
  }

  editModal(devis: any): void {
          this.selectedDevis = devis;
        
          this.editForm.patchValue({
            numDevis: devis.numDevis,
            tva: devis.tva,
            clientId: devis.client ? devis.client.id : null
          });
          ...
          devis.produits.forEach((prod: any) => {
      ...
      if (hasInvalidQuantity) {
        ...
        return;
            ...
            this.devisService.updateDevis(this.selectedDevis.id, formData, this.user.email).subscribe({
              next: () => {
                this.saveEdit();
                Swal.fire('Modifié!', "Le devis a été modifiée.", 'success');
                this.getAllDevis(this.user.entreprise.id);
                this.edit?.hide();
              },
              error: error => {
        ...
        deleteModule(factureId: number): void {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              this.devisService.deleteDevis(factureId).subscribe({
                next: () => {
                  Swal.fire('Supprimé!', "Le devis a été supprimée.", 'success');
                  this.getAllDevis(this.user.entreprise.id);
                },
                error: error => {
                  console.error(error);
        ...
        searchEmployeBy(): void {
          if (this.searchArticle.trim() === '') {
            ...
            this.devis = this.devis.filter(n =>
              n.numDevis.toLowerCase().includes(this.searchArticle.toLowerCase()) || n.client.nom.toLowerCase().includes(this.searchArticle.toLowerCase())
            );
          }
        }
        ...
        onDownloadFile(filename:string,numFacture:string){
          this.devisService.downloadFile(filename,this.user.email).subscribe(
            event=>{
              console.log(event);
        ...
        save(): void {
          ...
          const orderNumber = this.addForm.value.numDevis; // Use numCommande from the form
          ...
          this.clientService.getClientById(supplierId).subscribe(supplier => {
            ...
            pdfDoc.getBlob((blob: File) => {
              ...
              this.devisService.uploadFile(blob, this.attachementName).subscribe(
                (event: any) => {
        ...
        saveEdit(): void {
          // Récupérer les valeurs du formulaire
          const orderNumber = this.editForm.value.numDevis; // Use numCommande from the form
          ...
          return {
            content: [
              ...
              },
              {
                columns: [
                  [
                    { text: `Numéro de devis : ${orderNumber}`, bold: true },
