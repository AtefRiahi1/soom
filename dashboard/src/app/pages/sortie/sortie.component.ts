...
import { SortieService } from 'src/app/core/services/sortie.service';
...
export class SortieComponent {
  ...
  addForm: FormGroup;
          ...
          private fetchEmployeProfile(email: string): void {
            ...
            this.getAllSorties(data.entreprise.id);
          ...
          getAllSorties(idEnt: number): void {
            this.sortieService.getSortiesByEntrepriseId(idEnt).subscribe(sorties => {
          ...
          onAddSortie(): void {
            if (this.addForm.valid) {
              ...
              const formData = { ...this.addForm.value };
                  formData.nomFichier = `${formData.numSortie}.pdf`;
        
              this.sortieService.createSortie(formData,this.user.entreprise.id, this.addForm.value.clientId,this.user.email) // Replace with actual data
                .subscribe({
                  next: () => {
                    this.save();
                    Swal.fire('Ajouté!', 'La sortie a été ajouté avec succès.', 'success');
                    this.getAllSorties(this.user.entreprise.id);
                    this.add?.hide();
                    this.addForm.reset();
                  },
                  error: error => {
                    console.error(error);
                    Swal.fire('Erreur!', 'Une erreur est survenue lors de l\'ajout du sortie.', 'error');
                  }
                });
            }
          }
        
          editModal(sortie: any): void {
                  this.selectedSortie = sortie;
                
                  this.editForm.patchValue({
                    numSortie: sortie.numSortie,
                    tva: sortie.tva,
                    clientId: sortie.client ? sortie.client.id : null
                  });
                  ...
                  sortie.produits.forEach((prod: any) => {
              ...
              if (hasInvalidQuantity) {
                ...
                return;
                    ...
                    this.sortieService.updateSortie(this.selectedSortie.id, formData, this.user.email).subscribe({
                      next: () => {
                        this.saveEdit();
                        Swal.fire('Modifié!', "La sortie a été modifiée.", 'success');
                        this.getAllSorties(this.user.entreprise.id);
                        this.edit?.hide();
                      },
                      error: error => {
                ...
                deleteModule(factureId: number): void {
                  Swal.fire({
                    ...
                    if (result.isConfirmed) {
                      this.sortieService.deleteSortie(factureId).subscribe({
                        next: () => {
                ...
                searchEmployeBy(): void {
                  if (this.searchArticle.trim() === '') {
                    ...
                    this.sorties = this.sorties.filter(n =>
                      n.numSortie.toLowerCase().includes(this.searchArticle.toLowerCase()) || n.client.nom.toLowerCase().includes(this.searchArticle.toLowerCase())
                    );
                  }
                }
                ...
                onDownloadFile(filename:string,numFacture:string){
                  this.sortieService.downloadFile(filename,this.user.email).subscribe(
                    event=>{
                      console.log(event);
                ...
                save(): void {
                  ...
                  const orderNumber = this.addForm.value.numSortie; // Use numCommande from the form
                  ...
                  this.clientService.getClientById(supplierId).subscribe(supplier => {
                    ...
                    pdfDoc.getBlob((blob: File) => {
                      ...
                      this.sortieService.uploadFile(blob, this.attachementName).subscribe(
                        (event: any) => {
                ...
                saveEdit(): void {
                  // Récupérer les valeurs du formulaire
                  const orderNumber = this.editForm.value.numSortie; // Use numCommande from the form
                  ...
                  return {
                    content: [
                      ...
                      },
                      {
                        columns: [
                          [
                            { text: `Numéro de sortie : ${orderNumber}`, bold: true },
