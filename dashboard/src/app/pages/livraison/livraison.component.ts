   ...
   addForm: FormGroup;
      ...
      constructor(
        ...
        this.addForm = this.fb.group({
          numLivraison: ['', Validators.required],
      ...
      private fetchEmployeProfile(email: string): void {
        ...
        this.getAllLivraisons(data.entreprise.id);
      ...
      getAllLivraisons(idEnt: number): void {
        this.livraisonService.getLivraisonsByEntrepriseId(idEnt).subscribe(livraisons => {
      ...
      onAddLivraison(): void {
        if (this.addForm.valid) {
          ...
          const formData = { ...this.addForm.value };
              formData.nomFichier = `${formData.numLivraison}.pdf`;
    
          this.livraisonService.createLivraison(formData,this.user.entreprise.id, this.addForm.value.clientId,this.user.email) // Replace with actual data
            .subscribe({
              next: () => {
                this.save();
                Swal.fire('Ajouté!', 'La livraison a été ajouté avec succès.', 'success');
                this.getAllLivraisons(this.user.entreprise.id);
                this.add?.hide();
                this.addForm.reset();
              },
              error: error => {
                console.error(error);
                Swal.fire('Erreur!', 'Une erreur est survenue lors de l\'ajout du livraison.', 'error');
              }
            });
        }
      }
    
      editModal(livraison: any): void {
              this.selectedLivraison = livraison;
            
              this.editForm.patchValue({
                numLivraison: livraison.numLivraison,
                tva: livraison.tva,
                clientId: livraison.client ? livraison.client.id : null
              });
              ...
              livraison.produits.forEach((prod: any) => {
          ...
          if (hasInvalidQuantity) {
            ...
            return;
                ...
                this.livraisonService.updateLivraison(this.selectedLivraison.id, formData, this.user.email).subscribe({
                  next: () => {
                    this.saveEdit();
                    Swal.fire('Modifié!', "La livraison a été modifiée.", 'success');
                    this.getAllLivraisons(this.user.entreprise.id);
                    this.edit?.hide();
                  },
                  error: error => {
            ...
            deleteModule(factureId: number): void {
              Swal.fire({
                ...
                if (result.isConfirmed) {
                  this.livraisonService.deleteLivraison(factureId).subscribe({
                    next: () => {
            ...
            searchEmployeBy(): void {
              if (this.searchArticle.trim() === '') {
                ...
                this.livraisons = this.livraisons.filter(n =>
                  n.numLivraison.toLowerCase().includes(this.searchArticle.toLowerCase()) || n.client.nom.toLowerCase().includes(this.searchArticle.toLowerCase())
                );
              }
            }
            ...
            onDownloadFile(filename:string,numFacture:string){
              this.livraisonService.downloadFile(filename,this.user.email).subscribe(
                event=>{
                  console.log(event);
            ...
            save(): void {
              ...
              const orderNumber = this.addForm.value.numLivraison; // Use numCommande from the form
            ...
            saveEdit(): void {
              // Récupérer les valeurs du formulaire
              const orderNumber = this.editForm.value.numLivraison; // Use numCommande from the form
              ...
              return {
                content: [
                  ...
                  },
                  {
                    columns: [
                      [
                        { text: `Numéro de livraison : ${orderNumber}`, bold: true },
          ...
          getCRMPermissions(user:any) {
            ...
            convertToFacture(facture: any): void {
              ...
              this.factureService.convertirLivraisonEnFacture(facture.id,this.user.email).subscribe({
                  next: (response) => {
                      this.savefacture(response);
                      Swal.fire('Succès!', 'Livraison convertie en facture.', 'success');
                      this.getAllLivraisons(this.user.entreprise.id); // Refresh the list if needed
                  },
                  error: (error) => {
